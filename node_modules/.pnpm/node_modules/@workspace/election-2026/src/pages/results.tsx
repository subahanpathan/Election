    ...
    <div className="container max-w-6xl mx-auto px-4 py-8 space-y-10">
      ...
      {results.isFinalized && results.winner && (
        <motion.div 
          ...
          className="glass-card rounded-3xl p-8 md:p-12 relative overflow-hidden border-yellow-500/30 shadow-[0_0_40px_rgba(245,158,11,0.2)]"
        >
          ...
          <div className="text-center mb-16 relative z-10">
            ...
            >
              <Trophy className="w-12 h-12 text-yellow-400 mx-auto mb-4 drop-shadow-[0_0_15px_rgba(250,204,21,0.6)]" />
            </motion.div>
            <h2 className="text-3xl md:text-4xl font-heading font-bold text-gradient-gold tracking-widest uppercase">
              Elected Leaders
            </h2>
          </div>

          <div className="flex flex-col md:flex-row justify-center gap-16 md:gap-24 items-end relative z-10">
            {results.winner.headBoy && (
              <div className="flex flex-col items-center group w-full md:w-auto">
                <span className="text-xs tracking-widest text-muted-foreground uppercase mb-6 font-semibold">Head Boy</span>
                <div className="relative mb-6">
                  <div className="w-36 h-36 rounded-full bg-gradient-to-br from-yellow-400 to-amber-600 p-1 shadow-[0_0_30px_rgba(245,158,11,0.3)] group-hover:scale-105 transition-transform duration-300">
                    <div className="w-full h-full rounded-full bg-background overflow-hidden border-4 border-background flex items-center justify-center text-4xl font-bold font-heading text-muted-foreground">
                      {results.winner.headBoy.photoUrl ? (
                        <img src={results.winner.headBoy.photoUrl} alt={results.winner.headBoy.name} className="w-full h-full object-cover" />
                      ) : (
                        results.winner.headBoy.name
                          .split(" ")
                          .map(i => i[0])
                          .slice(0, 2)
                          .join("")
                  ...
                  <div className="absolute top-0 right-0 w-10 h-10 bg-yellow-500 rounded-full flex items-center justify-center border-2 border-background shadow-lg">
                    <Crown className="w-5 h-5 text-white" />
                  </div>
                </div>
                <h3 className="text-2xl font-bold font-heading mb-3 text-center">{results.winner.headBoy.name}</h3>
                <div className="px-4 py-1 bg-yellow-500/20 text-yellow-400 border border-yellow-500/30 rounded-full text-xs font-bold mb-2">
                  ELECTED
                </div>
                <p className="text-muted-foreground text-sm">
                  {results.headBoy.find(i => i.candidate.id === results.winner?.headBoy?.id)?.voteCount || 0} votes
                </p>
              </div>
            )}

            {results.winner.headGirl && (
              <div className="flex flex-col items-center group w-full md:w-auto mt-12 md:mt-0">
                <span className="text-xs tracking-widest text-muted-foreground uppercase mb-6 font-semibold">Head Girl</span>
                <div className="relative mb-6">
                  <div className="w-36 h-36 rounded-full bg-gradient-to-br from-yellow-400 to-amber-600 p-1 shadow-[0_0_30px_rgba(245,158,11,0.3)] group-hover:scale-105 transition-transform duration-300">
                    <div className="w-full h-full rounded-full bg-background overflow-hidden border-4 border-background flex items-center justify-center text-4xl font-bold font-heading text-muted-foreground">
                      {results.winner.headGirl.photoUrl ? (
                        <img src={results.winner.headGirl.photoUrl} alt={results.winner.headGirl.name} className="w-full h-full object-cover" />
                      ) : (
                        results.winner.headGirl.name
                          .split(" ")
                          .map(i => i[0])
                          .slice(0, 2)
                          .join("")
                ...
                <h3 className="text-2xl font-bold font-heading mb-3 text-center">{results.winner.headGirl.name}</h3>
                <div className="px-4 py-1 bg-yellow-500/20 text-yellow-400 border border-yellow-500/30 rounded-full text-xs font-bold mb-2">
                  ELECTED
                </div>
                <p className="text-muted-foreground text-sm">
                  {results.headGirl.find(i => i.candidate.id === results.winner?.headGirl?.id)?.voteCount || 0} votes
                </p>
              </div>
            )}
          </div>
            ...
            <div key={result.candidate.id} className={`flex items-center gap-4 p-3 rounded-xl ${idx === 0 ? 'bg-white/5 border border-white/10' : ''}`}>
              ...
              <div className="flex-1">
                ...
                <Progress 
                  ...
                  indicatorClassName={idx === 0 ? undefined : "bg-white/30"}
