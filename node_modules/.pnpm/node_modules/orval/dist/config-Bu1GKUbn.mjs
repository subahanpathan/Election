...
import { DefaultTag, FormDataArrayHandling, GetterPropType, NamingConvention, OutputClient, OutputHttpClient, OutputMockType, OutputMode, PropertySortOrder, RefComponentSuffix, SupportedFormatter, asyncReduce, buildDynamicScope, buildSchemaTagMap, collectReferencedComponents, compareNatural, conventionName, createSchemaOutputPlanForOutput, createSuccessMessage, dynamicImport, escapePath, filterPathsBySchemas, fixCrossDirectoryImports, fixRegularSchemaImports, generateComponentDefinition, generateDependencyImports, generateMutator, generateParameterDefinition, generateSchemasDefinition, generateVerbsOptions, getBaseUrlRuntimeImports, getConfiguredMockDirectories, getFileInfo, getFullRoute, getImportExtension, getMockFileExtensionByTypeName, getOperationUrlHelperNames, getRefInfo, getRoute, isBoolean, isComponentRef, isFunction, isNullish, isObject, isReference, isString, isUrl, jsDoc, kebab, log, logError, logVerbose, logWarning, normalizeRuntimeValidation, operationZodOverrideKeys, pascal, removeFilesAndEmptyFolders, resolveInstalledVersions, resolveRef, resolveValue, splitSchemasByType, stripFileExtension, upath, withGeneratedFileTransform, writeGeneratedFile, writeRoutedSchemas, writeSchemas, writeSchemasTagsSplit, writeSingleMode, writeSplitMode, writeSplitTagsMode, writeTagsMode, writeTagsOperationsMode, writeTagsOperationsSplitMode } from "@orval/core";
...
function assertSchemasPathIsDirectory(path, option) {
	if (!SOURCE_FILE_EXTENSIONS.has(nodePath.extname(path).toLowerCase())) return;
...
function normalizeOperationsAndTags(operationsOrTags, workspace, global, source) {
	return Object.fromEntries(Object.entries(operationsOrTags).map(([key, { transformer, mutator, formData, formUrlEncoded, paramsSerializer, paramsFilter, query, angular, zod, effect, ...rest }]) => {
		...
		if (unsupportedOperationZodKeys && unsupportedOperationZodKeys.length) {
			...
			logWarning(`⚠️  override.${source}.${key}.zod only supports ${supportedZodKeysLabel}. Ignoring unsupported ${fieldLabel}: ${unsupportedFields}.`);
		}
...
function normalizeQueryOptions(queryOptions = {}, outputWorkspace, globalOptions = {}) {
	...
	if (!isNullish(queryOptions.shouldExportQueryKey)) logWarning("⚠️  shouldExportQueryKey is deprecated and will be removed in a future major release. Please use shouldExportKeys instead, which also gates the mutation key getters.");
	...
	return async (input, init) => {
		...
		for (let hop = 0;; hop++) {
			...
			if (!isFetchableUrl(next, origin, allowedExternalRefs)) {
				logWarning(`Refused to follow a redirect to a URL that is not allowed: ${next}\nReached by redirect from ${url}.\nAdd it to externalRefs.allow or use ['*'] to allow all.`);
		...
		if (isObject(extDoc) && "components" in extDoc) {
			...
			if (isObject(extComponents) && "schemas" in extComponents) {
				...
				for (const [schemaName, schema] of Object.entries(extSchemas)) {
					...
					else if (schemaName in mainSchemas && !isMatchingXExtRef) finalSchemaName = `${schemaName}_${suffix}`;
					const isExistingPlaceholder = finalSchemaName === schemaName && isMatchingXExtRef;
					if (Object.hasOwn(mainSchemas, finalSchemaName) && !isExistingPlaceholder) throw new Error(`External schema "${schemaName}" from "${extKey}" cannot be merged as "${finalSchemaName}" because that component name is already occupied.`);
...
function generateZodSchemasFromVerbs(verbOptions, schemasPath, fileExtension, output, context, schemaTagMap, schemaOutputPlan) {
	...
	const generateVerbsSchemas = verbOptionsArray.flatMap((verbOption) => {
		...
		const responseSchemas = shouldGenerate.response ? [...verbOption.response.types.success, ...verbOption.response.types.errors].map((responseType) => {
			...
			if (hadArraySuffix) {
				while (cleanSchema && "type" in cleanSchema && cleanSchema.type === "array" && cleanSchema.items && !isReference(cleanSchema.items) && "type" in cleanSchema.items && !("$ref" in cleanSchema.items)) cleanSchema = cleanSchema.items;
...
async function writeZodSchemasSingle(builder, verbOptions, schemasPath, fileExtension, header, output, context, paramsMutator) {
	...
	await writeGeneratedFile(nodePath.join(schemasPath, `index${fileExtension}`), `${header}${getZodSchemaImportStatement(output.override.zod.variant)}\n\n${components}${operations}`);
}
//#endregion
...
async function writeSpecsInternal(builder, workspace, options, projectName) {
	...
	if (output.workspace) {
		...
		if (output.schemas) {
			const schemasPath = isString(output.schemas) ? output.schemas : output.schemas.path;
			imports.push(schemaOutputPlan ? upath.getRelativeImportPath(indexFile, schemaOutputPlan.basePath) : upath.getRelativeImportPath(indexFile, getFileInfo(schemasPath).dirname));
		}
...
//# sourceMappingURL=config-Bu1GKUbn.mjs.map