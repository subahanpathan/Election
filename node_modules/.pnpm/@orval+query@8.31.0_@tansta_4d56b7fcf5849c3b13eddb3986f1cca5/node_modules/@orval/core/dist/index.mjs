...
*/
function toLiteralTypeValue(value) {
...
function getScalar({ item, name, context, formDataContext }) {
	...
	switch (itemType) {
		...
		case "integer": {
			...
			let isEnum = false;
			if (enumItems) {
				value = enumItems.map((enumItem) => toLiteralTypeValue(enumItem)).join(" | ");
				isEnum = true;
			}
			value += nullable;
			if (schemaConst !== void 0) value = `${toLiteralTypeValue(schemaConst)}${nullable}`;
			return {
				value,
				isEnum,
				type: "number",
		...
		case "boolean": {
			let value = "boolean";
			if (enumItems && !(enumItems.includes(true) && enumItems.includes(false))) value = enumItems.map((enumItem) => toLiteralTypeValue(enumItem)).join(" | ");
			value += nullable;
			if (schemaConst !== void 0) value = `${toLiteralTypeValue(schemaConst)}${nullable}`;
			return {
				value,
