...
import { Vote, Users, AlertCircle, CheckCircle2 } from "lucide-react";
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";

export default function Dashboard() {
  const { student, hasVoted } = useAuth();
    ...
    <div className="container max-w-6xl mx-auto px-4 py-8 space-y-12">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 pb-6 border-b border-white/10">
        ...
        >
          ...
          <p className="text-muted-foreground mt-1">Class {student?.className} • Student ID: {student?.studentId}</p>
        </motion.div>
      ...
      {hasVoted ? (
        ...
        >
          <Alert className="bg-emerald-500/10 border-emerald-500/20 text-emerald-500">
            ...
            <AlertTitle className="text-lg font-heading font-semibold">Vote Recorded Successfully</AlertTitle>
            <AlertDescription className="text-emerald-500/80">
              Thank you for participating in the election! Your voice has been heard. 
              <div className="mt-4">
                <Button asChild variant="outline" className="bg-emerald-500/20 hover:bg-emerald-500/30 border-emerald-500/30 text-emerald-500">
        ...
        >
          ...
          <h2 className="text-2xl font-heading font-bold mb-2">You haven't voted yet</h2>
